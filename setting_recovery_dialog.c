...
static void recovery_enter(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    switch (lParam)
    {
    case 0:
...
static LRESULT setting_recovery_dialog_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    HDC hdc;
    ...
    case MSG_TIMER:
    {
        ...
        if (wParam == _ID_TIMER_SETTING_RECOVERY)
        {
#ifdef ENABLE_BATT
            if (batt != battery)
            {
...
//==================display volume icon============================
        ...
        DrawText(hdc, res_str[RES_STR_TITLE_RESTORE], -1, &msg_rcTitle, DT_TOP);
        FillBox(hdc, TITLE_LINE_PINT_X, TITLE_LINE_PINT_Y, TITLE_LINE_PINT_W, TITLE_LINE_PINT_H);

        page = (RECOVERY_NUM + SETTING_NUM_PERPAGE - 1) / SETTING_NUM_PERPAGE;
        ...
        {
            RECT msg_rcFilename;
            if ((cur_page * SETTING_NUM_PERPAGE + i) >= RECOVERY_NUM)
                break;
            ...
            SelectFont(hdc, logfont);
            DrawText(hdc, res_str[RES_STR_OK + cur_page * SETTING_NUM_PERPAGE + i], -1, &msg_rcFilename, DT_TOP);
        }
    ...
    case MSG_DESTROY:
        KillTimer(hWnd, _ID_TIMER_SETTING_RECOVERY);
        unloadres();
        break;
    ...
    case MSG_LBUTTONUP:
    {
        ...
        if (witch_button > 0 && witch_button < WHOLE_BUTTON_NUM)
        {
            ...
            InvalidateRect(hWnd, &msg_rcBg, TRUE);
            recovery_enter(hWnd, wParam, list_sel);
        }
...
void creat_setting_recovery_dialog(HWND hWnd)
{
    ...
    DialogBoxIndirectParam(&DesktopDlg, hWnd, setting_recovery_dialog_proc, 0L);
}
