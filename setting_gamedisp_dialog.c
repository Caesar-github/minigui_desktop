...
static LRESULT setting_eq_dialog_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case MSG_INITDIALOG: {
        ...
        SetTimer(hWnd, _ID_TIMER_SETTING_EQ, 100);
        return 0;
    }
    case MSG_TIMER: {
    ...
    case MSG_PAINT:
    {
        ...
        for (i = 0; i < GAMEDISP_NUM; i++) {
            ...
            DrawText(hdc, res_str[RES_STR_TITLE_GAMEDISP_FULL + i], -1, &msg_rcFilename, DT_TOP);
        }
        ...
        switch (wParam) {
            ...
            case KEY_UP_FUNC:
                 if (list_sel > 0)
                    ...
                    list_sel = GAMEDISP_NUM - 1;
                InvalidateRect(hWnd, &msg_rcBg, TRUE);
                break;
            case KEY_ENTER_FUNC:
                gamedisp_val = list_sel;
                InvalidateRect(hWnd, &msg_rcBg, TRUE);
                break;
        }
        break;
