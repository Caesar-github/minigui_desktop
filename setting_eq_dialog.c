...
static void eq_enter(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    set_eq(lParam);
...
static LRESULT setting_eq_dialog_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    HDC hdc;
    ...
    case MSG_INITDIALOG:
    {
        ...
        batt = battery;
        list_sel = 0;
        SetTimer(hWnd, _ID_TIMER_SETTING_EQ, TIMER_SETTING_EQ);
        return 0;
    }
    case MSG_TIMER:
    {
...
//==================display volume icon============================
        ...
        page = (EQ_NUM + SETTING_NUM_PERPAGE - 1) / SETTING_NUM_PERPAGE;
        ...
        for (i = 0; i < SETTING_NUM_PERPAGE; i++)
        {
            ...
            if ((cur_page * SETTING_NUM_PERPAGE + i) >= EQ_NUM)
                break;
            ...
            if ((cur_page * SETTING_NUM_PERPAGE + i) == get_eq())
            ...
            DrawText(hdc, res_str[RES_STR_EQ_1 + cur_page * SETTING_NUM_PERPAGE + i], -1, &msg_rcFilename, DT_TOP);
        }
...
void creat_setting_eq_dialog(HWND hWnd)
{
    ...
    DialogBoxIndirectParam(&DesktopDlg, hWnd, setting_eq_dialog_proc, 0L);
}
